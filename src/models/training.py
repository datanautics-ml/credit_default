...
class ModelTrainer:
    ...
    def _setup_models(self) -> Dict[str, Any]:
        ...
        models = {
            ...
            "ridge": {"model": RidgeClassifier(), "params": {"alpha": [0.1, 1.0, 10.0, 100.0]}},
            "svc": {"model": SVC(probability=True), "params": {"C": [0.1, 1.0, 10.0], "kernel": ["linear", "rbf"]}},
            # # 'lasso': {
            # #     'model': Lasso(),
            # #     'params': {
            ...
            "gradient_boosting": {
                ...
                "params": {"n_estimators": [100, 200], "learning_rate": [0.05, 0.1, 0.2], "max_depth": [3, 5, 7]},
            },
            "xgboost": {
    ...
    def prepare_data(
        self, features_df: pd.DataFrame, target_column: str = "default payment next month", test_size: float = 0.2
        ...
        exclude_columns = [
            target_column,
            "ID",
            "MONTH",  # Exclude categorical and IDs
    ...
    def train_model(
        self, model_name: str, X_train: pd.DataFrame, y_train: pd.Series, use_hyperparameter_optimization: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        with mlflow.start_run(run_name=f"{model_name}_training"):
            ...
            # Store trained model
            self.trained_models[model_name] = {"model": best_model, "params": best_params, "cv_score": best_score}
            ...
            return {"model": best_model, "best_params": best_params, "cv_score": best_score}
        ...
        for model_name in self.models.keys():
            try:
                if model_name in ["logistic_regression", "ridge", "svc"]:
                    ...
                    # Train model
                    training_result = self.train_model(model_name, X_train_scaled, y_train, use_hyperparameter_optimization)
                    # Evaluate model
                    evaluation_result = self.evaluate_model(model_name, X_test_scaled, y_test)

                    # Combine results
                    all_results[model_name] = {"training": training_result, "evaluation": evaluation_result}
                else:
                    # Train model
                    training_result = self.train_model(model_name, X_train, y_train, use_hyperparameter_optimization)
